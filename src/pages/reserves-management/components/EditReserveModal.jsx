...
const EditReserveModal = ({ isOpen, reserve, onClose, onSave }) => {
  ...
  useEffect(() => {
    if (reserve) {
      setFormData({
        ...
        items: reserve.items.map(item => ({ ...item })),
              ...
              <div>
                ...
                <div className="space-y-4">
                  {formData.items.map((item, index) => (
                    <div key={item.id || index} className="bg-secondary-50 rounded-lg p-4 border border-secondary-200">
                      <div className="flex items-center justify-between mb-4">
